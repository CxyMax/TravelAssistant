...
export async function generateTripPlan(formData) {
    try {
        const response = await apiClient.post('/trip/plan', formData);
 ...
 */
export async function refineTripPlan(session_id, feedback) {
    try {
        const response = await apiClient.post('/trip/refine', {
            session_id,
            feedback
        });
        return response.data;
...
export async function searchPoi(keywords, city) {
    try {
        const response = await apiClient.get('/poi/search', { params: { keywords, city, limit: 10 } });
