...
def maps_weather(city: str) -> Dict[str, Any]:
    """根据城市名称或者标准adcode查询指定城市的天气 返回城市未来几天的天气预报信息"""
    try:
        response = requests.get(
...
def maps_direction_transit_integrated_by_coordinates(origin: str, destination: str, city: str, cityd: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # [本地修补] 上游 0.1.11 在此处有一行 `print(data)`,在 stdio 传输下会把
        # Python dict repr 写入 stdout,破坏 JSON-RPC 协议流,导致客户端解析崩溃。
        # 已移除该 print。如需调试请用 logging 输出到 stderr,切勿写 stdout。
...
@mcp.tool()
def maps_text_search(keywords: str = "", city: str = "", citylimit: str = "false", types: str = "") -> Dict[str, Any]:
    """关键词搜索 API 根据用户输入的关键字进行 POI 搜索，并返回相关的信息

    Args:
        keywords: 搜索关键词(可为空,配合 types 按类别搜索)
        city: 城市名或 adcode
        citylimit: "true" 仅返回 city 内结果
        types: 高德 POI 分类编码,多个用 "|" 分隔(如 "110000" 风景名胜、"050000" 餐饮)。
               可为空。关键词为空时,可仅按 types 搜索某一类别的全部 POI。
    """
    try:
        response = requests.get(
...
def maps_around_search(location: str, radius: str = "1000", keywords: str = "", types: str = "") -> Dict[str, Any]:
    ...
    Args:
        location: 中心点经纬度 "经度,纬度"
        radius: 搜索半径(米)
        keywords: 关键词(可为空,配合 types 按类别搜索)
        types: 高德 POI 分类编码,多个用 "|" 分隔。可为空。
    """
    try:
        response = requests.get(
