"""高德 POI 分类编码表(官方 V1.06, 2023-02-08)。

数据来源: https://a.amap.com/lbs/static/code_resource/Amap_poicode.zip
  文件: 高德POI分类与编码(中英文)_V1.06_20230208.xlsx, 共 915 条。
自动生成,请勿手改。每个 6 位 NEW_TYPE 映射到 (大类, 中类, 小类)。
"""

from typing import Optional

# code -> (big 大类, mid 中类, sub 小类)
POI_TYPECODE = {
    ...
    "023301": ("汽车销售", "长城汽车销售", "魏派汽车销售"),
    "023400": ("汽车销售", "纳智捷销售", "纳智捷销售"),
    "023500": ("汽车销售", "广汽传祺销售", "广汽传祺销售"),
    ...
    "033401": ("汽车维修", "长城汽车维修", "魏派汽车维修"),
    "033500": ("汽车维修", "纳智捷维修", "纳智捷维修"),
    "033600": ("汽车维修", "广汽传祺维修", "广汽传祺维修"),
...
# ---- 反查辅助: 中文类名 -> code 列表(用于按类别搜索) ----
# 大类名 -> 大类 code(前两位+0000)。例: "餐饮服务" -> "050000"
_BIG_TO_CODE: dict = {}
# 中类名 -> 中类 code(前四位+00)。例: "咖啡厅" -> "050500"
_MID_TO_CODE: dict = {}
for _code, (_big, _mid, _sub) in POI_TYPECODE.items():
    if _code.endswith("0000"):
        _BIG_TO_CODE.setdefault(_big, _code)
    elif _code.endswith("00"):
        _MID_TO_CODE.setdefault(_mid, _code)


def decode_typecode(typecode: str, level: str = "mid") -> str:
    """把高德 typecode 解码成中文类别名。

    输入:
        typecode —— maps_text_search/around_search 返回的 typecode,
                    可能形如 "050500" 或 "110201|140100"(多类用 | 分隔)。
        level    —— "big" 大类 / "mid" 中类 / "sub" 小类。默认 "mid"。

    输出:
        对应级别的中文名;多 code 时取第一个;未知返回 "其他"。
        例: decode_typecode("050501")          -> "咖啡厅"   (mid)
            decode_typecode("050501","sub")    -> "星巴克咖啡"
            decode_typecode("110201|140100")   -> "风景名胜"
    """
    if not typecode:
        return "其他"
    first = str(typecode).split("|", 1)[0].strip()
    entry = POI_TYPECODE.get(first) or POI_TYPECODE.get(first[:2] + "0000")
    if entry is None:
        return "其他"
    big, mid, sub = entry
    return {"big": big, "mid": mid, "sub": sub}.get(level, mid)


def category_to_code(name: str) -> Optional[str]:
    """把中文类别名转成可用于搜索的 types code(大类或中类均可)。

    例: category_to_code("风景名胜") -> "110000"
        category_to_code("咖啡厅")   -> "050500"
        category_to_code("不存在")   -> None
    用法: 把返回值作为 search_poi(..., types=code) 传入。
    """
    if not name:
        return None
    return _BIG_TO_CODE.get(name) or _MID_TO_CODE.get(name)
